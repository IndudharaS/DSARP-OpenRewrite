...
equivalent Slurm mode is `pretrained_model`:

```bash
sbatch --export=ALL,PIPELINE_MODE=pretrained_model,STOP_STAGE=prediction \
  hpc/noctua_pipeline.sbatch
```
...
changes. A completed retraining run publishes its `final_model` to the shared
default using a lock and atomic directory replacement; the previous default is
retained as `shared/trained-model/default-backup-<timestamp>`.
